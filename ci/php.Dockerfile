...
RUN APP_ENV=prod SENTRY_DSN="" composer install --no-interaction --no-progress --no-suggest --optimize-autoloader --classmap-authoritative --no-cache --no-dev  --no-scripts
ADD ci/php.ini /usr/local/etc/php/php.ini
ADD ci/php_env .env
ADD ci/www.conf /usr/local/etc/php-fpm.d/www.conf
ADD . .
RUN APP_ENV=prod SENTRY_DSN="" composer install --no-interaction --no-progress --no-suggest --optimize-autoloader --classmap-authoritative --no-cache --no-dev
